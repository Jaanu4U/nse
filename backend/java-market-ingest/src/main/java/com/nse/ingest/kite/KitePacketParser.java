...
public class KitePacketParser {
    ...
    private TickDto parsePacket(byte[] pkt) {
        ...
        b.get();                           // [4] tradable flag – unused here
        b.get();                           // [5] mode flag – unused here

        double ltp  = b.getInt() / 100.0; // [6-9]
        ...
        if (pkt.length >= 44) {
            lastQty    = b.getInt() & 0xFFFFFFFFL; // [10-13]
            avgPrice   = b.getInt() / 100.0;        // [14-17]
            volume     = b.getInt() & 0xFFFFFFFFL;  // [18-21]
            totalBuyQ  = b.getInt() & 0xFFFFFFFFL;  // [22-25]
            totalSellQ = b.getInt() & 0xFFFFFFFFL;  // [26-29]
            open       = b.getInt() / 100.0;        // [30-33]
            high       = b.getInt() / 100.0;        // [34-37]
            low        = b.getInt() / 100.0;        // [38-41]
            close      = b.getInt() / 100.0;        // [42-45]
        }

        if (pkt.length >= 184) {
            ts = b.getInt() & 0xFFFFFFFFL; // [46-49] last trade time
            b.getInt();                    // [50-53] OI
            b.getInt();                    // [54-57] OI day high
            b.getInt();                    // [58-61] OI day low
            b.getInt();                    // [62-65] exchange ts

            // Depth: bids at [66], asks at [126]
            // 5 bid levels
            long bidQtySum = 0;
            double firstBidPrice = Double.NaN;
            long   firstBidQty  = 0;
            for (int j = 0; j < 5; j++) {
                long   dq = b.getInt() & 0xFFFFFFFFL;  // qty
                double dp = b.getInt() / 100.0;         // price
                b.getShort();                           // orders
                b.getShort();                           // padding
                bidQtySum += dq;
